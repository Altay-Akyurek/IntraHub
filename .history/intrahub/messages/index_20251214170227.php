...
<div class="container mt-4 fade-in-up">
    <div class="row" style="display: grid; grid-template-columns: 320px 1fr; gap: 1.5rem; height: calc(100vh - 150px); min-height: 500px;">
        <!-- Users List -->
        <div class="glass-card" style="padding: 0; overflow: hidden; display: flex; flex-direction: column;">
            <div style="padding: 1.5rem; border-bottom: 1px solid rgba(255,255,255,0.05); display: flex; justify-content: space-between; align-items: center; background: rgba(0,0,0,0.2);">
                <h5 style="margin: 0; font-weight: 700; background: linear-gradient(to right, var(--primary-color), var(--secondary-color)); -webkit-background-clip: text; -webkit-text-fill-color: transparent;">Sohbetler</h5>
            ...
            <div style="padding: 0 1rem 1rem 1rem;">
                <input type="text" id="userSearch" class="form-control mt-3" placeholder="Sohbetlerde Ara..." style="margin-bottom: 0;">
            </div>
            
            <div class="user-list" style="overflow-y: auto; flex: 1;" id="activeChatsList">
                <?php foreach ($users as $u): ?>
                    <a href="#" class="user-item" data-id="<?= $u['id'] ?>" data-username="<?= e($u['full_name'] ?: $u['username']) ?>">
                        <div class="avatar"><?= strtoupper(substr($u['username'], 0, 1)) ?></div>
                        <div class="user-info">
                            <div class="name"><?= e($u['full_name'] ?: $u['username']) ?></div>
                            <small class="text-muted"><?= e($u['role']) ?></small>
                        </div>
                    </a>
                <?php endforeach; ?>
            </div>
        ...
        <!-- Chat Area -->
        <div class="glass-card" id="chatContainer" style="display: flex; flex-direction: column; position: relative; height: 100%; overflow: hidden;">
            <div class="chat-header" id="chatHeader" style="display: none; padding: 1rem; border-bottom: 1px solid rgba(255, 255, 255, 0.05); margin-bottom: 0; background: rgba(0,0,0,0.2);">
                <h5 style="margin: 0; font-weight: 700; background: linear-gradient(to right, var(--primary-color), var(--secondary-color)); -webkit-background-clip: text; -webkit-text-fill-color: transparent;" id="chatTitle">Kullanıcı Seçin</h5>
            </div>
            
            <div id="messagesArea" style="flex: 1; overflow-y: auto; display: flex; flex-direction: column; gap: 1rem; padding-right: 10px;">
            ...
            <div class="chat-input-area" id="inputArea" style="display: none; margin-top: 1rem; border-top: 1px solid var(--border-color); padding-top: 1rem;">
                <form id="messageForm" style="display: flex; gap: 10px;">
                    <input type="text" name="content" class="form-control" placeholder="Bir mesaj yazın..." style="margin-bottom: 0;" required autocomplete="off">
...
<!-- New Chat Modal -->
<div class="modal-overlay" id="newChatModal" style="display: none; position: fixed; top:0; left:0; width:100%; height:100%; background: rgba(0,0,0,0.5); z-index: 1000; justify-content: center; align-items: center;">
    <div class="glass-card" style="width: 100%; max-width: 400px; margin: 2rem;">
        <h5 class="mb-3" style="font-weight: 700; background: linear-gradient(to right, var(--primary-color), var(--secondary-color)); -webkit-background-clip: text; -webkit-text-fill-color: transparent;">Yeni Sohbet Başlat</h5>
        <input type="text" id="modalSearch" class="form-control mb-3" placeholder="Kullanıcı adı ara..." style="background: rgba(0,0,0,0.2);">
        <div id="modalUserList" style="max-height: 300px; overflow-y: auto;">
            <?php foreach ($users as $u): ?>
                <div class="d-flex align-items-center p-2 border-bottom user-select-item" style="cursor: pointer; border-color: rgba(255,255,255,0.1) !important;" onclick="startChat(<?= $u['id'] ?>, '<?= e($u['full_name'] ?: $u['username']) ?>')">
                    <div class="avatar" style="width: 32px; height: 32px; font-size: 0.8rem; margin-right: 10px;"><?= strtoupper(substr($u['username'], 0, 1)) ?></div>
                    <div><?= e($u['full_name'] ?: $u['username']) ?></div>
                </div>
            <?php endforeach; ?>
        </div>
        <div class="mt-3 text-end">
            <button class="btn btn-secondary btn-sm" onclick="document.getElementById('newChatModal').style.display='none'">Kapat</button>
        </div>
    </div>
</div>
