...
if ($action === 'attend') {
    ...
    $row = $stmt->fetch();
    if ($row) {
        //toggle to  attendance if not attending
        if ($row['status'] === 'attending') {
            $upd = $pdo->prepare("UPDATE event_attendance SET status = 'not_attending', responded_at = NOW() WHERE id = :id");
            $upd->execute(['id' => $row['id']]);
            echo json_encode(['ok' => true, 'status' => 'not_attending']);

        } else {
            $upd = $pdo->prepare("UPDATE event_attendance SET status = 'attending', responded_at = NOW() WHERE id = :id");
            $upd->execute(['id' => $row['id']]);
            echo json_encode(['ok' => true, 'status' => 'attending']);
        }
    } else {
        $ins = $pdo->prepare("INSERT INTO event_attendance (event_id, user_id, status, responded_at) VALUES (:eid,:uid,'attending',NOW())");
